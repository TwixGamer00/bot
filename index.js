...
if (!config.welcomeID) {
	console.error("You need to specify the welcome channel id in the config.json file!");
	process.exit(1);
}
...
client.on("guildMemberAdd", async member => {
	if (!member.user.bot) {
		...
		const guild12 = await client.guilds.fetch(config.guildID);
		var welcomeCanvas = new canvacordD.Welcome();

		let welcomeCard = await welcomeCanvas
        .setUsername(member.user.username)
        .setDiscriminator(member.user.discriminator)
        .setMemberCount(guild12.members.cache.filter(member => !member.user.bot).size)
        .setGuildName(guild12.name)
        .setAvatar(avatarURL)
		...
		.setBackground("https://images.unsplash.com/photo-1579546929662-711aa81148cf?ixlib=rb-1.2.1&ixid=MXwxMjA3fDB8MHxleHBsb3JlLWZlZWR8MXx8fGVufDB8fHw%3D&w=1000&q=80")
		.toAttachment();
            var image = new Discord.MessageAttachment(welcomeCard.toBuffer(), "welcome-image.png");
			client.channels.cache.get(config.welcomeID).send("**Someone joined the server!**");
			client.channels.cache.get(config.welcomeID).send(image);
...
client.on("guildMemberRemove", async member => {
	if (!member.user.bot) {
		const avatarURL = await client.getProfilePic(member.user);
		const guild12 = await client.guilds.fetch(config.guildID);
		var byeCanvas = new canvacordD.Goodbye();

		let byeCard = await byeCanvas
        .setUsername(member.user.username)
        .setDiscriminator(member.user.discriminator)
        .setMemberCount(guild12.members.cache.filter(member => !member.user.bot).size)
        .setGuildName(guild12.name)
		...
		.setBackground("https://images.unsplash.com/photo-1579546929662-711aa81148cf?ixlib=rb-1.2.1&ixid=MXwxMjA3fDB8MHxleHBsb3JlLWZlZWR8MXx8fGVufDB8fHw%3D&w=1000&q=80")
		.toAttachment();
            var image = new Discord.MessageAttachment(byeCard.toBuffer(), "welcome-image.png");
			client.channels.cache.get(config.welcomeID).send("**Someone left the server!**");
			client.channels.cache.get(config.welcomeID).send(image);
	}
});
